    ...
    <div class="dropdown-menu">
        @can('access_sale_payments')
            <a href="{{ route('sale-return-payments.index', $data->id) }}" class="dropdown-item">
                <i class="bi bi-cash-coin mr-2 text-warning" style="line-height: 1;"></i> Detail Pembayaran
            </a>
        @endcan
        @can('access_sale_payments')
            @if($data->due_amount > 0)
                <a href="{{ route('sale-return-payments.create', $data->id) }}" class="dropdown-item">
                    <i class="bi bi-plus-circle-dotted mr-2 text-success" style="line-height: 1;"></i> Pembayaran
                </a>
            @endif
        @endcan
        @can('edit_sales')
